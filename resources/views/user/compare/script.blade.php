<script>
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success' && response.data.length === 0) {
                        toastr.error('No vehicles found with the selected criteria.');
                        return;
                    ...
                    response.data.forEach(vehicle => {
                        ...
                        let vehicleName =
                            `${vehicle.make_name ?? ''} ${vehicle.model_name ?? ''} ${vehicle.variant_name ?? ''} ${vehicle.year ?? ''}`
                            .trim();
                        let firstImage = (vehicle.images?.split(',')[0] || 'no-image.jpg')
                            .trim();
                        ...
                        if (vehicle.other_vehicles && vehicle.other_vehicles.length > 0) {
                            vehicle.other_vehicles.forEach(v => {
                                let img = v.images ? v.images.split(',')[0].trim() :
                                    '';
                                let text =
                                    `${v.make_name} ${v.model_name} ${v.variant_name ?? ''} (${v.year ?? ''})`;
                                selectorOptions +=
                                    `<option value="${v.id}" data-auction="${vehicle.auction_id}" data-image="${img}">${text}</option>`;
                            });
                        } else {
                            selectorOptions +=
                                `<option disabled>No other vehicles</option>`;
                        }
                    ...
                    $('.vehicle-selector').each(function() {
                        $(this).select2({
                            ...
                            templateResult: function(state) {
                                ...
                                if (img) {
                                    return $(
                                        `<span style="display:flex; align-items:center;"><img src="${img}" style="width:50px; height:auto; margin-right:8px; border-radius:4px;"> ${state.text}</span>`
                                    );
                                }
                                return state.text;
                            },
        ...
        function loadComparisonBody(vehicleIds = []) {
            ...
            if (vehicleIds.length === 0) {
                body.append(
                    '<tr><td colspan="100%" style="text-align:center; padding:20px; font-size:16px; color:red;">No Vehicles Selected</td></tr>'
                );
                return;
            }
