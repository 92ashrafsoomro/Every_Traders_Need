<template>
    ...
    <div v-for="(item, index)  in masterStore.fuelType.data" :key="item.id" class="d-flex align-center justify-space-between" >
           <v-checkbox 
            v-if="showAll || index <= 10"
             v-model="auctionStore.filter.f" 
             :label="item.label" 
             :value="item.id"
             @change="handleChange"
             />
    ...
    <div class="py-4">
          <p style="text-decoration: underline ; cursor: pointer;" @click="showAll = !showAll" v-if="masterStore.fuelType.data.length > 10">
