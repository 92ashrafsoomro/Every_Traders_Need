<template>
    ...
    <div v-for="(item, index)  in masterStore.grades.data" :key="item.id" class="d-flex align-center justify-space-between" >
           <v-checkbox 
            v-if="showAll || index <= 10"
             v-model="auctionStore.filter.grade" 
             :label="item.label" 
             :value="item.id"
             @change="handleChange"
             />
    ...
    <div class="py-4">
          <p style="text-decoration: underline ; cursor: pointer;" @click="showAll = !showAll" v-if="masterStore.grades.data.length > 10">
