        ...
        <div class="v-navigation-drawer__content">
            <v-list density="compact" nav>
                <v-list-item class="d-flex" style="height: 57px;">
                    <img v-if="menuWidth == 269" :src="currentLogo"
                ...
                <!-- Open Sidebar -->
                <div class="position-fixed d-lg-flex d-md-flex d-none align-center cursor-pointer bg-primary"
                    v-if="!subMenuOpen" style="width: 20px; height: 54px; border-radius: 0 10px 10px 0; z-index: 100; left: 70px; top: 80px; " @click="toggleSubSidebar">
                ...
                <template v-for="(item, index) in userMenu" :key="index">
                    ...
                    <v-list-item v-else :to="item.path" link :ripple="false" class="text-subtitle-1"
                        :class="{ 'bg-primary on-primary rounded my-active-menu hide-overlay': $route.path === item.path }"
                        @click="item.hasOwnProperty('children') ? openSubSidebar(item) : closeSubSidebar()">
                        <template #prepend>
