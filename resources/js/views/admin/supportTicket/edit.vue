<template>
  <v-container fluid class="bg-background pa-0 align-start" max-width="1600px" >
    <v-sheet border="bottom" class="d-flex align-center w-100 bg-surface px-6 py-3 sticky-header" elevation="1">
      <v-btn icon="mdi-arrow-left" variant="text" @click="goBack" class="mr-2" color="light"></v-btn>
      <v-avatar size="40" class="mr-3 border">
        <v-img       :src="ticket.replies[0]?.user?.avatar 
        ? `https://localhost/autoboli/public/uploads/avatar/${ticket.replies[0].user.avatar}` 
        : 'https://cdn.vuetifyjs.com/images/lists/1.jpg'"></v-img>
      </v-avatar>
      <div>
        <h1 class="text-subtitle-1 font-weight-bold mb-0">{{ ticket.user_name }}</h1>
        <!-- <div class="d-flex align-center">
          <v-badge dot color="success" inline class="mr-1"></v-badge>
          <span class="text-caption text-success">Active</span>
        </div> -->
      </div>
      <v-spacer></v-spacer>
      ...
      <v-col class="d-flex flex-column chat-main-col border-r border-border">
        ...
        <div class="messages-viewport flex-grow-1 overflow-y-auto pa-6" ref="chatWindow">
          ...
          <div v-for="reply in ticket.replies" :key="reply.id" 
               :class="['d-flex mb-6', reply.is_admin ? 'justify-end' : 'justify-start']">
            
            <div :class="['d-flex align-start max-w-75', reply.is_admin ? 'flex-row-reverse' : 'flex-row']">
              ...
              <div :class="['bubble-container d-flex flex-column', reply.is_admin ? 'align-end' : 'align-start']">
                <div class="d-flex align-center mb-1 px-1">
                  <span class="text-caption font-weight-bold text-light mr-2">{{ reply.is_admin ? 'Beasley' : ticket.user_name }}</span>
                  <span class="text-disabled" style="font-size: 0.65rem">{{ formatDate(reply.created_at) }}</span>
                </div>
                
                <v-card 
                  :class="['chat-bubble elevation-1 overflow-hidden', reply.is_admin ? 'admin-bubble text-white' : 'client-bubble bg-surface text-white']"
                  variant="flat"
    ...
    <v-col cols="12" md="3" class="bg-surface d-none d-md-flex flex-column info-sidebar pa-6 overflow-y-auto border-l">
      <div class="text-center mb-6">
        ...
        <p class="text-caption text-disabled">{{ ticket.replies[0]?.user?.personalEmail }}</p>
        
        <div class="d-flex justify-center ga-3 mt-3">
          <v-btn icon="mdi-phone" variant="tonal" size="x-small" color="primary" :href="'tel:' + ticket.replies[0]?.user?.phone"></v-btn>
          <v-btn icon="mdi-email-outline" variant="tonal" size="x-small" color="primary" :href="'mailto:' + ticket.replies[0]?.user?.personalEmail"></v-btn>
          <v-btn v-if="ticket.replies[0]?.user?.website" icon="mdi-earth" variant="tonal" size="x-small" color="primary" :href="ticket.replies[0]?.user?.website" target="_blank"></v-btn>
        </div>
      </div>
      ...
      <div class="mb-6">
        ...
        <v-list density="compact" class="bg-transparent pa-0">
          ...
          <v-list-item class="px-0 min-h-0 mb-3">
            ...
            <v-list-item-subtitle class="text-body-2 text-white font-weight-bold">
              {{ ticket.user_name || 'N/A' }} (ID: {{ ticket.user_id }})
      ...
      <div class="flex-grow-1">
        <span class="text-overline font-weight-black text-primary">Business Details</span>
        <v-list density="compact" class="bg-transparent pa-0 mt-2">
          <v-list-item class="px-0" prepend-icon="mdi-briefcase-outline">
            <v-list-item-title class="text-caption text-disabled">Company & Type</v-list-item-title>
            <v-list-item-subtitle class="text-body-2 text-white">
              {{ ticket.replies[0]?.user?.companyName }} ({{ ticket.replies[0]?.user?.businessType }})
            </v-list-item-subtitle>
          </v-list-item>

          <v-list-item class="px-0" prepend-icon="mdi-map-marker-outline">
            <v-list-item-title class="text-caption text-disabled">Location</v-list-item-title>
            <v-list-item-subtitle class="text-body-2 text-white">
              {{ ticket.replies[0]?.user?.townCity }}, {{ ticket.replies[0]?.user?.country }}
            </v-list-item-subtitle>
          </v-list-item>
        </v-list>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchTicketDetails() {
      this.loading = true;
      try {
        const res = await General.get(`/api/cruds/tickets/${this.$route.params.id}`);
    ...
    async submitReply() {
      ...
      try {
        ...
        await General.post(`/api/cruds/tickets/${this.ticket.id}/reply`, payload);
