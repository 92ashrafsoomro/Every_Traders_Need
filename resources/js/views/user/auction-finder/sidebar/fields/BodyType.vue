<template>
    ...
    <div v-for="(item, index)  in masterStore.bodyTypes.data" :key="item.id" class="d-flex align-center justify-space-between" >
           <v-checkbox 
            v-if="showAll || index <= 7"
             v-model="auctionStore.bodyTypes.bodyType" 
             :label="item.label" 
             :value="item.id"
             @change="handleChange"
             />
    ...
    <div class="py-4">
          <p style="text-decoration: underline ; cursor: pointer;" @click="showAll = !showAll" v-if="masterStore.bodyTypes.data.length > 7">
